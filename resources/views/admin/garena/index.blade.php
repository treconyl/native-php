...
@section('content')
    <header>
        <h2 class="text-3xl font-semibold text-slate-900 mb-0!">Account Test Runner</h2>
    ...
    <div class="sunrise-card p-6 space-y-4">
        ...
        <p class="text-xs text-slate-500">Chọn tài khoản đã import, nhập mật khẩu mới và chọn proxy (nếu cần) trước khi chạy
            Playwright.</p>
        <form method="POST" action="{{ route('admin.garena.credentials') }}" class="space-y-3">
            ...
            <div>
                ...
                <input type="text" name="new_password" value="{{ old('new_password', $credential->new_password ?? '') }}"
                    class="sunrise-input mt-1" placeholder="Password#2025" required>
                <span class="sunrise-help mt-0">
                    Gợi ý: 8-16 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt. Tránh dùng chuỗi dễ đoán như ngày
                    sinh.
                </span>
            </div>
            <div>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                ...
                <button type="submit" formaction="{{ route('admin.garena.run') }}" class="btn-sunrise">Chạy test</button>
            </div>
    ...
    <div class="sunrise-card p-6 w-full overflow-hidden space-y-3">
        ...
        <div class="overflow-auto">
            <pre
                class="text-xs bg-slate-900 text-slate-100 rounded-2xl p-4 max-w-full w-full max-h-112 whitespace-pre-wrap wrap-break-word">{{ implode("\n", $logLines) }}</pre>
        </div>
    </div>
@endsection